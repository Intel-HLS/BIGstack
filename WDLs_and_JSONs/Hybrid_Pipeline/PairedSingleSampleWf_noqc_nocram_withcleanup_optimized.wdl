...
task SamtoolsSort {
  ...
  command <<<
    ${tool_path}/samtools sort -m ${mem_limit} --threads ${samtools_threads} -l ${compression_level} ${input_bam} -o ${output_bam_basename}.bam
 ...
 # While query-grouped isn't actually query-sorted, it's good enough for MarkDuplicates with ASSUME_SORT_ORDER="queryname"
  command {
    java -Dsamjdk.compression_level=${compression_level} -Xmx${java_heap_memory_initial} -jar ${tool_path}/picard.jar \
      ...
      ADD_PG_TAG_TO_READS=false && \
      rm -rf ../inputs && \
      rm -rf ../../call-SamToFastqAndBwaMemAndMba && \
      rm -rf ../../call-GetBwaVersion && \
      rm -rf ../../call-CheckFinalVcfExtension
  }
  runtime {
    memory: memory
    cpu: cpu
...
task GatherBqsrReports {
  ...
  command {
    ${tool_path}/gatk4/gatk-launch --javaOptions "-Dsamjdk.compression_level=${compression_level} -Xmx${java_heap_memory_initial}" \
      GatherBQSRReports \
...
task GatherBamFiles {
  ...
  command {
    java -Dsamjdk.compression_level=${compression_level} -Xmx${java_heap_memory_initial} -jar ${tool_path}/picard.jar \
      ...
      rm -rf ../../call-SortSampleBam && \
      rm -rf ../../call-CreateSequenceGroupingTSV
...
task ValidateSamFile {
  ...
  command {
    java -Dsamjdk.compression_level=${compression_level} -Xmx${java_heap_memory_initial} -jar ${tool_path}/picard.jar \
      ...
      rm -rf ../inputs && \
      rm -rf ../../call-ConvertToCram && \
      rm -rf ../../call-CramToBam
  }
  runtime {
    memory: memory
    cpu: cpu
...
task HaplotypeCaller {
  ...
  command {
    ${tool_path}/gatk4/gatk-launch --javaOptions "-Dsamjdk.compression_level=${compression_level} -Xmx${printreads_java_heap_memory_initial}" \
      PrintReads \
      -I ${input_bam} \
      ...
      -O local.sharded.bam \
    && \
    java -XX:GCTimeLimit=50 -XX:GCHeapFreeLimit=10 -Dsamjdk.compression_level=${compression_level} -Xmx${haplotypecaller_java_heap_memory_initial} \
      -jar ${tool_path}/GenomeAnalysisTK.jar \
      -T HaplotypeCaller \
      -R ${ref_fasta} \
...
task MergeVCFs {
  ...
  command {
    ...
    rm -rf ../inputs && \
    rm -rf ../../call-ScatterIntervalList && \
    rm -rf ../../call-CheckContamination && \
    rm -rf ../../call-GatherBamFiles && \
	rm -rf ../../call-HaplotypeCaller
  }
  runtime {
    memory: memory
    cpu: cpu
...
task ValidateGVCF {
  ...
  command {
    ...
    rm -rf ../inputs && \ 
    rm -rf ../../call-GatherBamFiles && \
    rm -rf ../../call-MergeVCFs
    #rm -rf ../../call-SumFloats && \
  }
  runtime {
    memory: memory
    cpu: cpu
