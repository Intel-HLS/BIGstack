...
do
        echo $i
        curl -vXPOST http://$CROMWELL_HOST:8000/api/workflows/v1 -F workflowSource=@${WDL} -F workflowInputs=@${JSON}_${i}.json -F workflowDependencies=@$BASEDIR/warp.zip > 20k_submission_response.txt
