...
task SamToFastqAndBwaMem {
  ...
  command <<<
    ...
    bwa_threads=${bwa_threads}
  	...
  	java -Xmx${java_heap_memory} -jar /usr/gitc/picard.jar \
    	SamToFastq \
    	INPUT=${input_bam} \
    	FASTQ=/dev/stdout \
    	INTERLEAVE=true \
...
task SortAndFixTags {
  ...
  command {
    ...
    java -Xmx${sort_java_heap_memory} -jar /usr/gitc/picard.jar \
    SortSam \
    INPUT=${input_bam} \
    OUTPUT=/dev/stdout \
    ...
    java -Xmx${fix_tags_java_heap_memory} -jar /usr/gitc/picard.jar \
    SetNmAndUqTags \
    INPUT=/dev/stdin \
    OUTPUT=${output_bam_basename}.bam \
    CREATE_INDEX=true \
...
task GatherBqsrReports {
  ...
  command {
    java -Xmx${java_heap_memory} \
      -cp /usr/GenomeAnalysisTK.jar org.broadinstitute.gatk.tools.GatherBqsrReports \
